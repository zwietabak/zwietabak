## 👋 Hi there

My name is Oliver Klein and I'm a professional Systems and Network / DevOps Engineer with over 14 Years of experience in the IT industry. I planed, implemented and managed different types of systems, services, networks and critical infrastructure.

In my free time I'm a passionate video gamer. Through my studies I got insight in the different topics of game development like programing, graphics game design and sound design. Right now I focus on game development with the Godot engine as a hobbyist and participate regularly at Game Jams.
Here you can have a look on public projects I worked on.

---

### Korpsgeist

Korpsgeist is a first person story driven mystery game that was developed during my studies in about 4 months.

You play a young police officer, fresh out of the academy, who’s been assigned to the night shift. Your task is to get a prisoner to confess through conversations held across the bars of his holding cell. You can offer him favors, negotiate with him, threaten him, or play psychological mind games.

But as time goes on, you begin to sense that something is following you through the entire police station.

🎮 Play the latest [Release](https://hdm-stuttgart.de/medianight/projekte?projekt_ID=4869) <br>
<!-- 🎬 Watch the [Video walkthrough]() <br> -->

|<img src="/img/korpsgeist/korpsgeist_01.png"  width="387">|<img src="/img/korpsgeist/korpsgeist_02.png" width="387">
|---|---|
|<img src="/gif/korpsgeist/Korpsgeist_Trailer_Final.gif" width="387">|<img src="" width="387">|

#### ⚙ Technologies I used
- Godot 4.4
- GDScript
- Git
- Gimp
- Blender
- Material Maker
- Audacity

#### 💻 Tasks
- Full implementation of character control mechanics
- Design and development of the interaction system for the prisoner, objects, and environmental elements
- Implementation of a dynamic dialogue system featuring branching decision logic
- Development of the core gameplay logic, managing both prisoner and level states and coordinating interactions between all subsystems
- Design and optimization of the lighting system
- Integration of sound effects and background music
- Rigging and animation using Mixamo
- Performance optimization, with a focus on level loading times and resource management
- Setup of an automated 3D asset pipeline, including:
    - Automatic export from Maya
    - Automatic import and scene construction in Godot
- UI implementation
- Technical support for team member and collaboration across multiple project departments

---

### Bouce Ball: Neon Party Arcade

Bounce Ball: Neon Party Arcade is reimagining a videogame classic: Play virtual table tennis with up to 4 players in a futuristic visual style using new gameplay elements like power ups and randomly generated arenas. Experience a fast-paced multiplayer game in which even the arena can change from moment to moment. Will you keep cool and come out on top?

Bounce Ball: Neon Party Arcade has been developed by an interdisciplinary team of students as a semester project within a time frame of four months. We hope you enjoy our final results! 

🎮 Play the latest [Build](https://store.steampowered.com/app/1822300/Bounce_Ball_Neon_Party_Arcade) <br>
<!-- 🎬 Watch the [Video walkthrough]() <br> -->

|<img src="/img/bounce_ball/bounce_ball_logo.jpg" width="387">|<img src="/img/bounce_ball/bounce_ball_04.jpg" width="387">|
|---|---|
|<img src="/gif/bounce_ball/bounce_ball_01.gif" width="387">|<img src="/gif/bounce_ball/bounce_ball_02.gif" width="387">|

#### ⚙ Technologies I used:
- Unity 2012
- C#
- Git
- Confluence

#### 💻 Tasks
- Implementation of Arena Elements
- Random Arena Generation
- Particle Effects
- Game Design
- Gameplay Testing


---
## Game Jam Projects

### Gib ihm Gibbon

Play Gibbi in the zoo from an isometric view in a 3D-Environment. Gibbi doesn't like to get petted by humans so your job is it, to slap the visitors away by whirling Gibbis arm physicle correct with your mouse cursor.

This game was created in 48 hours during the Global Game Jam 2024 with the theme "Make me laugh". 

🎮 Play the latest [Build](https://fabiomangiameli.itch.io/gib-ihm-gibbon) <br>
<!-- 🎬 Watch the [Video walkthrough]() <br> -->

|<img src="/img/gibbon/gibbon01.png" width="387">|<img src="/img/gibbon/gibbon03.png" width="387">|
|---|---|
|<img src="/gif/gibbon/gibbon.gif" width="387">|<img src="" width="387">|

#### ⚙️ Technologies I used:
- Godot 4.2
- GDScript
- Git
- Gimp

#### 💻 Tasks
- Developing enemies
- Developing enemy spawner
- Sound implementation
- UI implementation
- Game Design
- Gameplay Testing

---

### Crunch Time

Crunch Time was created during the [BeansJam 2022](https://itch.io/jam/beansjam-22). The theme for this game jam was "We have to stay awake" and was developed in 48 hours. The game was rated place 7 out of 65 submissions!

The game is about a company late at night. You are the intern who needs to supply the workers with caffeinated drinks. Every now and then the sandman will come by and try to put your colleagues to sleep. Stop him by giving him a punch!
The more employees are awake, the more money is earned. The goal is to keep your colleagues awake all night.

🎮 Play the latest [Build](https://zwietabak.itch.io/crunch-time) <br>
<!-- 🎬 Watch the [Video walkthrough]() <br> -->

|<img src="/img/crunch_time/Crunch_Time_Menu.PNG"  width="387">|<img src="/img/crunch_time/Crunch_Time_In_Game.PNG" width="387">|
|---|---|
|<img src="/gif/crunch_time/Crunch_Time.gif" width="387">|<img src="/gif/crunch_time/Crunch_Time_Zoomed_In.gif" width="387">|

#### ⚙️ Technologies I used:
- Godot 3.5
- GDScript
- Git
- Gimp

#### 💻 Tasks
- Developing interactables
- UI implementation
- Sound implementation
- Game Design
- Gameplay Testing

---

### Scary Dark Dungeon

You wake up in a dark place. A dungeon which is located under your home town. Confused and in pain you try to figure out what is going on until you hear a friendly voice. A little fairy appears, glowing in the dark and floating around. She offers you help and together you try to escape from the scary dark dungeon.

This game was created in 48 hours during the miniBeansJam' 9 with the theme "We are not alone".

🎮 Play the latest [Build](https://fabiomangiameli.itch.io/scary-dark-dungeon) <br>
<!-- 🎬 Watch the [Video walkthrough]() <br> -->

|<img src="/img/dungeon/SSD_Screenshot_2.png"  width="387">|<img src="/img/dungeon/SSD_Screenshot_4.png" width="387">|
|---|---|
|<img src="/gif/dungeon/SDD_scene_1.gif" width="387">|<img src="/gif/dungeon/SDD_scene_2.gif" width="387">|

#### ⚙️ Technologies I used:
- Godot 4.2
- GDScript
- Git
- Gimp

#### 💻 Tasks
- Developing enemies
- Developing companion and combat mechanics
- Lighting and Postprocessing
- Environment Design
- Game Design
- Gameplay Testing
